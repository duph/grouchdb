...
Bridge.modify_node.cycle_graph.update_bridge_data = function(key, doc, prev_key, callback)
{
...
Bridge.modify_node.cycle_graph.visited = function(doc, prev_key, callback){
	// if (!(doc.bridge.back_edges.indexOf(prev_key) > -1) && !(doc.bridge.forward_edges.indexOf(prev_key) > -1))
	// {
		...
		doc.back_edges.push(prev_key);
		Bridge.modify_node.cycle_graph.update_bridge_data(key, doc, prev_key, callback);
		});
	// }
	// else
	// 	callback();
